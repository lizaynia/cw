...
public class AdminService {
    ...
    public AdminService(SessionFactory sessionFactory, UserDao userDao, RoleDao roleDao, AirplaneDao airplaneDao) {
    ...
    public AdminService() {
        this(HibernateUtil.getSessionFactory(), new UserDao(), new RoleDao(), new AirplaneDao());
    }
    ...
    public String toggleUserBlock(Integer userId, boolean block) {
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            transaction.commit();
            return "Успех: Статус блокировки пользователя изменен.";
        } catch (Exception e) {
            if (transaction != null) transaction.rollback();
            return "Ошибка блокировки: " + e.getMessage();
        }
    }
