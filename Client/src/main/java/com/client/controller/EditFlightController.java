...
public class EditFlightController extends BaseController {
    ...
    private Runnable onFlightUpdated;
    ...
    public void setFlight(FlightDto flight) {
        ...
        // Нужно также выбрать правильный самолет в комбобоксе
        // Это сделаем после загрузки списка самолетов
    }
    ...
    private void loadAirplanes() {
        ...
        executeTask(request, response -> {
            ...
            // Выбираем самолет, соответствующий оригинальному рейсу
            if (originalFlight != null && originalFlight.getAirplaneModel() != null) {
                airplaneComboBox.getSelectionModel().selectFirst(); // Упрощенно
                // В реальности нужно найти самолет по модели
    ...
    public void setOnFlightUpdated(Runnable onFlightUpdated) {
        this.onFlightUpdated = onFlightUpdated;
    }
    ...
    private void handleSave() {
        ...
        // Используем UPDATE_FLIGHT команду (нужно добавить в CommandType)
        Request request = new Request(CommandType.UPDATE_FLIGHT.name(),
                originalFlight.getId(), flightNum, depCity, arrCity, dateTime, airplane.getId(), price);

        executeTask(request, response -> {
            showInfo("Успех", "Рейс успешно обновлен!");
            if (onFlightUpdated != null) onFlightUpdated.run();
